...
// Запит для отримання даних одного кемпера
...
// Запит для отримання списку кемперів
  ...
  async (_, thunkAPI) => {
    ...
    try {
      while (!isLastPage && filteredCampers.length < currentPage * LIMIT) {
        const response = await axios.get(`/campers?page=${page}&limit=${LIMIT}${buildQueryParams()}`);
